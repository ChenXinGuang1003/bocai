...
$roundDate = $_GET['d'];//date("Y-m-d");//$roundDate = '2016-03-13';
...
$pageLimit = 400;// 每页最大记录数 最多500
*/
$roundDate = strtotime($roundDate);

$date1 = date('Y-m-d', $roundDate)." 00:00:00";
$date2 = date('Y-m-d H:i:s', $roundDate+24*3600);
//exit;

//$array_agents = explode(',',$_GET['u']);
//$nums = count($array_agents);

//echo $date1.$date2;exit;

if(true){

		$result0 = $bbinapi->GetRecords1($date1,$date2);
		
		$results = json_decode($result0,true);
...
if ($results['Success'] == 1){
	...
	$result = $results['Data'];
        ...
        if(count($result['Records'])>0){
			if(isset($result['Records']['client']))
			{
				$temp = $result['Records'];
				unset($result['Records']);
				$result['Records'][0]=$temp;
			}
            foreach ($result['Records'] as $key => $value){
				
                $sql = "select * from `agbetdetail` where billNo = '".$value['betNum']."'";
				//echo $sql;exit;
                ...
                try{
                    ...
                    $sql = "insert into  `agbetdetail`(`billNo`,`playerName`,`gameCode`,`netAmount`,`betTime`,`gameType`,`betAmount`,`validBetAmount`,`flag`,`playType`,`recalcuTime`,`platformType`,`round`,`tableCode`)".
                            ...
                            $value['gameRoundId']."','".
                            $value['winOrLoss']."','".
                            date("Y-m-d H:i:s",  strtotime($value['betTime']))."','".
                            $value['gameType']."','".
                            $value['validAmount']."','".
                            $value['validAmount']."','".
                            $value['Flag']."','".
                            ...
                            $value['Round']."','".
                            $value['tableName']."'".
                            " FROM DUAL WHERE EXISTS(SELECT `ab_username` FROM user_list WHERE `ab_username` = '".$value['client']."');";   
					//echo $sql;exit;
                    $mysqli->query($sql) or die($mysqli->error);
                    $total ++;
                    }  else { // 有记录就更新，因服务器有延时，结果应该为已经结算完成的，故先不更新，后续观察
                        //$sql = "update  `agbetdetail` set netAmount='".value['Payoff']."',flag='". value['Flag']."' where billNo = '".value['betNum']."'";
                    }
                 } catch (Exception $ex) {
                        die($ex->getMessage());
                }    
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td align="left">
    <input type=button name=button value="刷新" onClick="window.location.reload()"> 本次获取ALLBET投注记录
    <?php echo $total;?>条
