...
if($nums){
	for($i=0;$i<$nums;$i++){
		$result = $bbinapi->GetBetDetailByGame($array_agents[$i], $roundDate,$gameKind,$startTime,$endTime,$page0,$pageLimit);
            ...
            foreach ($result['Record'] as $key => $value){
                ...
                try{
                    ...
                    $sql = "insert into  `agbetdetail`(`billNo`,`playerName`,`gameCode`,`netAmount`,`betTime`,`gameType`,`betAmount`,`validBetAmount`,`flag`,`playType`,`recalcuTime`,`platformType`,`round`,`tableCode`)".
                            ...
                            $value['Round']."','".
                            $value['GameCode']."'".
                            " FROM DUAL WHERE EXISTS(SELECT `mg_username` FROM user_list WHERE `mg_username` = '".$value['UserName']."');";   
