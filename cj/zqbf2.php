...
if(intval(date('H'))<12)
{
	$bdate=date("m-d",strtotime("-2 days"));//判断详细时间
    $today = date("Y-m-d",strtotime("-2 days"));//接收日期
}
else
{
	...
	for ($ii=0;$ii<=$sc;$ii++){
		...
		if ($d[13]!=''){
			...
			$cou    = count($rows);
				...
				$sql="update bet_match set mb_inball='$mb_inball',tg_inball='$tg_inball',tg_inball_hr='$tg_inball_hr',mb_inball_hr='$mb_inball_hr' where match_id=$match_id  and Match_Date='$bdate'";
				$mysqli->query($sql);
