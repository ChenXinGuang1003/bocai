...
function butt()
{
	...
	for(var i=1;i<autourl.length;i++)
		document.write("<input type=text name=txt"+i+" size=10 value=测试中……>-><input type=text name=url"+i+" size=40>-><input type=button value=进入太阳城 onclick=window.open('"+autourl[i]+"','mainFrame') class='buttonopen hand'><hr size='1' />")
	document.write("</form>")
}

function auto(url)
{
	document.forms[0]["url"+b].value="网站"+b;
	if(tim>200)
