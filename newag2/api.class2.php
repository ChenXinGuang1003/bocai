 ...
 */
include_once '/newag2/http.class.php';
class BBIN_TZH
{
    private $url    = "http://47.88.8.241:741";
    private $comId   = "hg66g";
    private $comKey  = "ed74f05dfadf14e2";
    public $gamePlatform = "AGIN";
    public $debug = 0;



    public function BBIN_TZH($comId,$comKey,$gamePlatform="AGIN")
    {
        $this->comId    = $comId;
        $this->comKey   = $comKey;  
    ...
    public function GameUserLogin($username,$gameName='')
    {
        ...
        $receive = $http->post($this->url,$xml);
		//return $receive;
        $pregString = "#<form[\s\S]+</script>#";
        preg_match($pregString, $receive, $matches);
        return $matches[0];
        if($matches[1]=="100")
        {
            return $matches[2];
