...
if($lottery_type=="D3"){
    $gType = "d3";
}elseif($lottery_type=="P3"){
    $gType = "p3";
}elseif($lottery_type=="T3"){
    $gType = "t3";
}
...
<div class="round-table">
    ...
    FROM lottery_result_".$gType." WHERE 1=1 ";
        if($qishu_query){
            $sql .= " AND qishu = '$qishu_query'";
        }
        if($gType=="d3" || $gType=="p3"){
            $d3_time = date('Y-m-d',strtotime('-6 day')).' 00:00:00';
            $sql .= " AND datetime > '$d3_time'";
        }elseif($gType=="t3"){
            $sql .= " AND DATE_FORMAT(datetime,'%Y-%m-%d') = '$query_time'";
        }
        ...
        while($rows = $query->fetch_array()){
            ...
            <tr   class="R_tr" align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                ...
                <td><?=f3D_Auto($hm,1)?> / <?=(f3D_Auto($hm,2)=="总和大"?"<span style=\"color: red;\">".f3D_Auto($hm,2)."</span>":f3D_Auto($hm,2))?> / <?=(f3D_Auto($hm,3)=="总和双"?"<span style=\"color: red;\">".f3D_Auto($hm,3)."</span>":f3D_Auto($hm,3))?></td>
                <td><?=(f3D_Auto($hm,4)=="龙"?"<span style=\"color: red;\">".f3D_Auto($hm,4)."</span>":f3D_Auto($hm,4))?></td>
                <td><?=f3D_Auto($hm,5)?></td>
                <td><?=f3D_Auto($hm,6)?></td>
            </tr>
        <?php
