	...
	$.getJSON("json/ft_2_2.php?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
		...
		if(pagecount == 0){
			...
			for(var i=0; i<dbs.length; i++){
				...
				htmls +="  <td class='zhong'>"+((dbs[i]["Match_Total01Pl"] !=null && dbs[i]["Match_Total01Pl"] !="0")?"<a href=\"javascript:void(0)\" title=\"0~1\" onclick=\"setbet('足球早餐','入球数-0~1','" + dbs[i]["Match_ID"] + "','Match_Total01Pl','0',0,'0~1');\" style='"+(dbs[i]["Match_Total01Pl"]!=data[i]["Match_Total01Pl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFFF00":"")+"'>"+formatNumber(dbs[i]["Match_Total01Pl"],2)+"</a>":"")+"</td>";
				htmls +="  <td class='zhong'>"+((dbs[i]["Match_Total23Pl"] !=null && dbs[i]["Match_Total23Pl"] !="0")?"<a href=\"javascript:void(0)\" title=\"2~3\" onclick=\"setbet('足球早餐','入球数-2~3','" + dbs[i]["Match_ID"] + "','Match_Total23Pl','0',0,'2~3');\" style='"+(dbs[i]["Match_Total23Pl"]!=data[i]["Match_Total23Pl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFFF00":"")+"'>"+formatNumber(dbs[i]["Match_Total23Pl"],2)+"</a>":"")+"</td>";
				...
				htmls +="  <td class='zhong'>"+((dbs[i]["Match_Total7upPl"] !=null && dbs[i]["Match_Total7upPl"] !="0")?"<a href=\"javascript:void(0)\" title=\"7以上\" onclick=\"setbet('足球早餐','入球数-7UP','" + dbs[i]["Match_ID"] + "','Match_Total7upPl','0',0,'7UP');\" style='"+(dbs[i]["Match_Total7upPl"]!=data[i]["Match_Total7upPl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFFF00":"")+"'>"+formatNumber(dbs[i]["Match_Total7upPl"],2)+"</a>":"")+"</td>";
				htmls +="</tr>";
			}
			}
			htmls+="</table>";
