...
(function(space){
    ...
    var _nowGtype = 'D3';
    //多重限額rtype
    var _multAry = ['W2', 'W3', 'W32', 'W33', 'OEOU', 'KD', 'SN', 'MCS', 'MUS', 'CUS'];
    //option
    var _opt = {
        url : {
        ...
        selector : {
            grp_d1 : '#W1quick > p a.activeA',
            grp_d1_gold : '#qkGoldW1',
            grp_d2_head : '#D2quick > p a.activeA',
            grp_d2_end : '#D2quick > p a.activeB',
            grp_d2_gold : '#qkGold',
            grp_d2_tk : '#D2quick > p#jjomj',
            ...
            SC : 'input[name=SC]',
            SO : 'input[name=SO]',
            lineRtype : "input[name='aLineRtype[]']",
            ...
            cancel_active : '#SendB3 input.cancel_cen[name=Cancel]',
            submit_active : '#SendB3 input.order[name=SubmitA]',
            dt_show : '#limitShow'
        }
    };
    function back() {
        $.ajax({
            url : _nowGtype.replace(_opt.reg.back, _opt.url.back),
            dataType : 'script'
        });
    }
    ...
    space.betB3.onDragBet = function (_gold, _node) {
        ...
        _form['gid'].value = document.getElementById('formB3').gid.value;
        (self.betSpace) && self.betSpace.BetConfirm();
    };
    /**
     * 下注完成
     */
    space.betB3.onFinish = function () {
    };
    space.betB3.prototype = {
        //下注表單
        _form : null,
        //金額欄位
        _goldInp : {},
        ...
        init : function () {
            ...
            var _rtype = $(_selector.rtype, this._form),
                _gold = $(_selector.gold, this._form),
                _odds = $(_selector.odds, this._form),
                _gold_gmin = $(_selector.gold_gmin, this._form),
                _gold_gmax = $(_selector.gold_gmax, this._form),
                _SC = $(_selector.SC, this._form),
                _SO = $(_selector.SO, this._form),
                _lineRtype = $(_selector.lineRtype, this._form),
                _submit = $(_selector.submit, this._form),
                _cancel = $(_selector.cancel, this._form),
                _finish = $(_selector.finish, this._form),
                _print = $(_selector.print, this._form),
            ...
            var multCheck = ($.inArray(this.rtypeGrp, _multAry) >= 0);
            this._goldInp = {};
            this._oddsInp = {};
            var _ku = function () {
            ...
            _gold.each(function (i) {
                ...
                me._oddsInp[stringConcede] = _odds.get(i);
                if (multCheck) {
        ...
        initFill : function () {
            ...
            this._betNumb.each(function(i) {
                ...
                //this.onblur = new Function('NoBorderB3(' + i + ');');
                this.onblur = _blur; 
            });
            //輔助功能
...
//            }
        ...
        onBetAReset : function (me) {
            ...
            this.reset();
            (me._winGoldTag) && (me._winGoldTag.innerHTML = '0.00');
            (me._goldTag) && (me._goldTag.innerHTML = '0.00');
        },
        /**
         * 下注檢查,一字过关、组选三、组选六等类型的确定按钮
         * @param me space.betB3物件
         */
...
//                        }
        ...
        onW3Chk : function (me) {
            ...
            for (var k in me._goldInp) {
                ...
                if (inp.value != 'NN') {
                    ...
                    if (inp.value == '') {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('pls_key_amount') + "!!\n";}
                    ...
                    if (isNaN(parseInt(inp.value)) == true) {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('MSG_ONLY_INPUT_NUMBER') + "!!\n";}
                    ...
                    _win += Math.round(_gold * (_odds - 1) * 100) / 100;
                    if (_gold < _gMin) {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('amount_under') + _gMin+ "!!\n";}
                    ...
                    if (_gold > _gMax) {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('amount_max') + _gMax + "!!\n";}
                    ...
                    if (_gold > _SO) {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('amount_sg_bet') + _SO + "!!\n";}
                    ...
                    if (_total > me.credit) {
                        if(_LineRtype=='W3' && !W3_ERR){W3_ERR=true;sErr += Lang('W3') + Lang('amount_ov_credit') + me.credit + "!!\n";}
        ...
        onFillClick : function (me) {
            var _gold = $(this).prev(_opt.selector.gold).val();
            for (var k in me._goldInp) {
        ...
        onDelClick : function (me) {
            ...
            var _check = $(_opt.selector.concede, me._form).length;
            if (_check == 0) {
                back();
                return;
            }
        ...
        onFillNumBlur : function (me) {
            ...
            if(str.length == 3){
                ...
                var url = "oddsB3_action.php?rtype=" + this.form.rtype.value + "&num=" + str + "&gid=" + this.form.gid.value + "&gtype=" + this.form.gtype.value
                $.ajax({
                    url: url,
                    dataType : 'json',
        ...
        onFillOddsUpdate : function (me) {
            ...
            $.ajax({
                url:'odds2B3_action.php',
                type: 'POST',
                dataType: 'json',
                timeout: 5000,
                data:"B3act=D3MCU&gtype=" + me._form.gtype.value + "&rtype=D3MCU&gid=" + $("#gid").val() + '&' + aryNumb.join('&'),
                success: function (res) {
        ...
        onFillShowUpdate : function (me) {
            ...
            if (iLine < 5) {
                ...
                $.ajax({
                    url:'quickD3B3_action.php',
                    type: 'POST',
                    dataType: 'json',
                    timeout: 5000,
                    data:"gtype=" + me._form.gtype.value + "&rtype=D3MCU&lineN=" + iLine + "&gid=" + $("#gid").val(),
                    beforeSend:function(){
        ...
        onRandomClick : function (me) {
            $.ajax({
                ...
                dataType: 'json',
                timeout: 5000,
                data:"B3act=Random&rtype=" + me._form.rtype.value + "&gid=" + $("#gid").val() + "&sRow=" + me._betNumb.length + "&gtype=" + me._form.gtype.value,
                success: function (res) {
                    if (res[0] == 'N') {
                        me._betNumb.each(function (i) {
                            this.value = res[1][i];
        ...
        onD2HeadClick : function (me) {
            ...
            var _num = this.innerHTML, _g = self.GameB3.instance();
            if (this.x == 1) {
                this.x = 0;
        ...
        onD2EndClick : function (me) {
            ...
            var _g = self.GameB3.instance(), _num = this.innerHTML;
            if (this.x == 1) {
                this.x = 0;
                $(this).removeClass(_class);
     ...
     * instance
     */
    space.betB3.instance = function (_gtype) {
        if (!this._instance) {
            this._instance = new this(_gtype);
        }
