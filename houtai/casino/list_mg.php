...
<?php 
      ...
      $sql	=	"select id from agbetdetail where  platformType = 'MG' ";
	  ...
	  if($bid){
	  	$bid	=	rtrim($bid,',');
	  	$sql	=	"select a.*,b.user_name from  (select * from agbetdetail where id in($bid) order by betTime desc) a left join user_list b on a.playerName = b.mg_username ";
	  	$query	=	$mysqli->query($sql);
