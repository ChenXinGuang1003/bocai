...
<?php 
		...
		$sql .= " where (platformType = 'AGIN' or platformType = 'AG')  and  betTime >= '" . $_GET['date_start'] . "'";
		$sql .= " and betTime <= '" . $_GET['date_end'] . "'";
		if($_GET['username']) $sql .= " and playerName = '$user'";
		$sql .= " GROUP BY playerName ORDER BY cnt DESC) a left join user_list b on a.playerName = b.ag_username ";
		$query = $mysqli->query($sql);
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
	          ...
	          <td  align="center" ><?php echo $rows['playerName']?></td>
			   <td><a href="javascript:window.open('/newag2/cha2.php?u=<?php echo $rows['playerName']?>','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();">点击获取</a></td>
