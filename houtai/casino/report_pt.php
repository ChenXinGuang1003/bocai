...
if($_GET['username']) {
	...
	$sql="select pt_username from k_user where username = '" . $_GET['username'] . "'";
...
<?php 
		$sql = "SELECT COUNT(*) AS cnt, SUM(betAmount) AS ba, SUM(netAmount) AS na, SUM(validBetAmount) AS va, playerName FROM agbetdetail ";
		$sql .= " where platformType = 'PT' and  betTime >= '" . $_GET['date_start'] . "'";
		$sql .= " and betTime <= '" . $_GET['date_end'] . "'";
		if($_GET['username']) $sql .= " and playerName = '$user'";
		$sql .= " GROUP BY playerName ORDER BY cnt DESC";
		$query = $mysqli->query($sql);
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>;">
	          <td  align="center" ><?php echo $rows['playerName']?></td>
			   <td><a href="javascript:window.open('/newpt/cha.php?u=<?php echo $rows['playerName']?>','list','width=440,height=180,left=50,top=100,scrollbars=no').focus();">点击获取</a></td>
