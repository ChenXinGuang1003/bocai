...
if($_GET["action"]=="add" && $id==0){
    ...
    $query	=	$mysqli->query($sql);
	if($query){echo '<script>alert("删除成功"); window.href.location="/bh-100/Lottery/result/BJPK/result_bjpk.php?status=0&type=%E5%8C%97%E4%BA%ACPK%E6%8B%BE";</script>';}
	else{echo '<script>alert("删除失败"); window.href.location="/bh-100/Lottery/result/BJPK/result_bjpk.php?status=0&type=%E5%8C%97%E4%BA%ACPK%E6%8B%BE";</script>';}
